...
|---|---|---|---|
| `mbclaw-mother` | MBclaw 母体主线，包含 Gateway、Memory、Runtime、Planner、Scheduler、TokenPool 接入、控制面板 | 主线核心 | 最优先研究与接手 |
...
| `LiteLLM` | 模型网关、fallback、兼容层、路由 | TokenPool/模型入口参考 | 重点借鉴路由与冷却思路 |
...
| `FreeLLMAPI` | 兼容网关/中转思路 | TokenPool 旁路参考 | 仅保留可复用点 |
...
| `freellmapi` | 兼容 API 的早期参考 | TokenPool 历史参考 | 仅保留有用文档 |
...
5. **后续施工优先级：主线边界 → TokenPool 整理 → 交接摘要 → 归档清理**。
