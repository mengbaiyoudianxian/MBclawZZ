...
if not os.path.exists(_PANEL_PATH):
    _PANEL_PATH = os.path.join(os.path.dirname(__file__), "panel_one.html")
ADMIN_HTML = open(_PANEL_PATH).read() + "<!-- TS: " + str(__import__("time").time()) + " -->"
...
app.include_router(mother_runtime_router)
...
def _panel_js_path():
    p = "/opt/mbclaw/app/admin/panel.js"
    if os.path.exists(p):
        return p
    return os.path.join(os.path.dirname(__file__), "panel.js")

@app.get("/admin/panel.js", response_class=HTMLResponse)
def panel_js():
    return FileResponse(_panel_js_path(), media_type="application/javascript")
@app.get("/admin2/panel.js", response_class=HTMLResponse)
def panel_js2(): return FileResponse(_panel_js_path(), media_type="application/javascript")

@app.get("/admin2", response_class=HTMLResponse)
@app.get("/admin2/", response_class=HTMLResponse)
def admin2_panel():
    return HTMLResponse(content=ADMIN_HTML, media_type="text/html")
