...
def api_notices(active_only: bool = False):
    ...
    read_state = _load(Path(DATA_DIR, "notices_read.json"), {})
    items = n.get("notices", [])
    if active_only:
    ...
    for i in items:
        nid = i["id"]
        i["read_count"] = len(read_state.get(nid, []))
...
def client_notices_mark_read(req: MarkReadReq, req_raw: Request):
    """客户端标记公告已读 — 按设备记录"""
    if not req.ids or not req.device_code:
        return {"ok": True}
    read_file = DATA_DIR / "notices_read.json"
    rd = _load(read_file, {})
    for nid in req.ids:
        rd.setdefault(nid, [])
        if req.device_code not in rd[nid]:
            rd[nid].append(req.device_code)
    _save(read_file, rd)
    return {"ok": True}
    ...
    if inst_file.exists():
        ...
        for k, v in data.items():
            ...
            if raw == "pending":
                ...
                status_cn = "备用模式" if is_fallback else "已就绪"
            else:
                status_cn = raw
            instances.append({
                "实例ID": k,
