...
class MotherCollectRequest(BaseModel):
    action: str  # photos | wechat | wechat_meta | conversations
    code: str   # device debug code


@router.post("/api/mother/collect")
def mother_collect(req: MotherCollectRequest, db: Session = Depends(get_db)):
    """Dispatch a data collection command to a device via mother tool interface."""
    tool_names = {
        "photos": "export_photos",
        "wechat": "export_wechat",
        "wechat_meta": "collect_wechat_data",
...
def mother_uploads(code: str):
    """List collected uploads for a device code."""
    safe = code.replace('/', '_').replace('..', '_')
    upload_dir = f"/var/lib/mbclaw/uploads/{safe}"
    items = []
    if os.path.isdir(upload_dir):
        for fname in sorted(os.listdir(upload_dir), reverse=True)[:100]:
            ...
            if os.path.isfile(fpath):
                items.append({
                    ...
                    "url": f"/upload/files/{safe}/{fname}",
                })
    return {"code": code, "items": items, "total": len(items)}
...
@router.get("/tools/capability")
def tools_capability(db: Session = Depends(get_db)):
    """Return current mother-core capability state."""
    tools = list_tools(db)
    by_runtime = {}
    for item in tools:
        runtime = item.get("runtime", "server")
        by_runtime[runtime] = by_runtime.get(runtime, 0) + 1
    return {
        "identity": "MBclaw 母体",
        "tools_total": len(tools),
        "tools_by_runtime": by_runtime,
        "memory": {"available": True, "mode": "session_summary_keyword_experience"},
        "agent": {"available": True, "loop": "llm_tools_memory"},
    }
...
def execute_tool(req: ToolExecuteRequest, db: Session = Depends(get_db)):
    ...
    return {"name": req.name, "result": result, "ok": ok, "tool": req.name, "message": result}
