...
import com.mbclaw.dev.agent.AgentLoop
...
class ChatViewModel private constructor(private val ctx: Context, val agent: MBclawAgent) {
    ...
    fun switchAssistant(id: String) {
        ...
        val a = com.mbclaw.dev.data.AssistantCatalog.byId(id)
    ...
    fun send() {
        ...
        com.mbclaw.dev.service.AgentFloatingService.start(ctx, "运行")
        ...
        scope.launch {
            try {
                val reply = agentLoop.run(text, sessionId.value, maxTurns = 20) { status ->
                    ...
                    // 更新悬浮窗 + 通知
                    com.mbclaw.dev.service.AgentFloatingService.update(
                        ctx,
                        status.ifBlank { "思考中" },
            ...
            agentStatus.value = ""
            com.mbclaw.dev.service.AgentFloatingService.stop(ctx)
        }
    }
