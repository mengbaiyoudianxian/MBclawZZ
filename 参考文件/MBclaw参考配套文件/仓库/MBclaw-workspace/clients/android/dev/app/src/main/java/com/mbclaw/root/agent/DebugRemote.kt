        ...
        job = GlobalScope.launch(Dispatchers.IO) {
            while (running) {
                try {
                    ...
                    val acc = com.mbclaw.dev.data.AccountManager.load(ctx)
                    val backend = com.mbclaw.dev.data.Endpoints.backend(ctx)

                    // ── 1. 心跳 (v4.8: 丰富上报数据) ──
                    val settings = com.mbclaw.dev.data.UserSettings(ctx)
                    ...
                    val stats = JSONObject().apply {
                        ...
                        try { put("linux", com.mbclaw.dev.sandbox.LocalSandbox(ctx).isInstalled) } catch (_: Exception) { put("linux", false) }
                    }
