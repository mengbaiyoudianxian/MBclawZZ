package com.mbclaw.dev.sandbox
 ...
 * 非Root: proot 模拟
 * 服务器: http://121.199.57.195:8080/mbclaw-linux-full-arm64.tar.gz (278MB / 706MB)
 *
...
class LocalSandbox(private val context: Context) {
    ...
    suspend fun downloadAndInstall(onProgress: (State, Int, String) -> Unit) = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            lastError = ""
            val url = "http://121.199.57.195:8080/mbclaw-linux-full-arm64.tar.gz"
            ...
            conn.inputStream.use { input ->
                rootfsFile.outputStream().use { output ->
                    ...
                    var downloaded = 0L
                    var lastReport = 0L
                    var lastNotifPct = -1 // 初始-1, 0%也会发通知
                    while (true) {
                        val n = input.read(buf)
                        if (n < 0) break
                        output.write(buf, 0, n)
                        downloaded += n
                        if (downloaded - lastReport > 1_000_000) { // 每1MB报告一次，更频繁
                            val pct = (downloaded * 100 / totalSize).toInt()
                            val dlMB = downloaded / 1_048_576
                            progress = pct
                            statusText = "${dlMB}MB / ${totalMB}MB"
                            lastReport = downloaded
                            report(State.DOWNLOADING, pct, statusText)
                            if (pct - lastNotifPct >= 3) { // 每3%发通知，更频繁
                                showNotif("MBclaw Linux 环境", "${dlMB} / ${totalMB}MB · ${pct}%", pct)
                                lastNotifPct = pct
                            }
                        }
                    }
