...
class PermissionTier private constructor(private val context: Context) {
    ...
    fun shellRoot(cmd: String, timeoutMs: Long = 5000): String? {
        ...
        // ★ v5.0.7: sh -c可能返回权限错误(非空), 检测到权限失败时继续走su
        var result = execOne(arrayOf("sh", "-c", cmd))
        val shFailed = result != null && (
            result.contains("Permission denied") || result.contains("SecurityException") ||
            result.contains("Operation not permitted") || result.contains("not allowed") ||
            result.contains("Permission Denial") || result.contains("requires") && result.contains("permission") ||
            result.contains("not permitted") || result.contains("MANAGE_USERS") ||
            result.contains("INTERACT_ACROSS_USERS"))
        if (result != null && !shFailed) return result
        if (shFailed) android.util.Log.w("MBclaw-Tier", "sh blocked, falling through to su: ${result.take(80)}")
        // 多路径su兜底
